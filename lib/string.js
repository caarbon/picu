...
*/
utils.pluralize = function pluralize(num, string) {
 ...
 */
utils.pad = function pad(string, length, chr) {
  chr = chr || ' ';
 ...
 */
utils.replace = function replace(template, replacements, pattern) {
  pattern = pattern || /\{\{([^\}]+)\}\}/g;
  ...
  return template.replace(pattern, function(match, key) {
    ...
    if (callingFunc) {
      ...
      val = replacements[ callingFunc[2] ];
      return func(val);
    }

    return typeof replacements[key] === 'undefined' ? '' : replacements[key];
  });
};
 ...
 */
var leadingHash = /^#/;
utils.ensureHexColor = function ensureHexColor(color, noHash) {
